...
def save_stay_times(entries: List[Dict], source_default: str = "timemachine") -> int:
    ...
    try:
        ...
        for item in entries:
            ...
            departure_time = parse_datetime_safe(item.get("departure_time"))
            push_time = parse_datetime_safe(item.get("push_time")) or current_date
            source = item.get("source", source_default)

            if not patient_id or not arrival_time or not departure_time:
        ...
        existing_records = db.session.query(PatientStayTime).all()
        existing_keys = {(r.patient_id, r.arrival_time) for r in existing_records}
        new_entries = [e for e in parsed_entries 
                      if (e['patient_id'], e['arrival_time']) not in existing_keys]
        
        if not new_entries:
            return 0  # No new entries

        # Process new entries - save raw data only
        for entry in new_entries:
            # Calculate difference timedelta for the difference field
        ...
        db.session.commit()
        logger.info(f"Saved {saved_count} stay_time records (raw data only).")
        return saved_count
