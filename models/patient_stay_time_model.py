...
class PatientStayTime(db.Model):
    ...
    departure_time = db.Column(db.DateTime, nullable=False, comment='Time of patient departure')
    difference = db.Column(db.DateTime, nullable=False, comment='Length of stay (hh:mm:ss)')
    ...
    __table_args__ = (
        db.UniqueConstraint('patient_id', 'arrival_time', 'departure_time', name='uix_patient_stay'),
    )
